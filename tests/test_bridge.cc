...
TEST(shadow_rate_tracking) {
  ShadowTracker tracker;
  NeuronArray digital;
  digital.Resize(4);
  // 3 out of 4 digital neurons spiking (75%)
  digital.spiked[0] = 1; digital.spiked[1] = 1;
  digital.spiked[2] = 1; digital.spiked[3] = 0;

  // Only 1 out of 4 bio neurons spiking (25%)
  std::vector<BioReading> bio = {
    {0, 0.9f, 0, std::nanf("")},
    {1, 0.1f, 0, std::nanf("")},
    {2, 0.1f, 0, std::nanf("")},
  ...
  auto snap = tracker.Measure(digital, bio, 100.0f);
  assert(snap.digital_spike_rate > 0.7f);  // ~0.75
  assert(snap.bio_spike_rate < 0.3f);      // ~0.25
  assert(snap.rate_ratio > 2.5f);          // 3:1 ratio
  assert(tracker.RateDiverged(5.0f) == false);  // within 5x
  assert(tracker.RateDiverged(2.0f) == true);   // outside 2x
}

TEST(shadow_drift_accelerating) {
  ShadowTracker tracker;
  NeuronArray digital;
  digital.Resize(2);

  // Build 40 history entries: first 20 with high correlation, next 20 dropping
  for (int i = 0; i < 20; ++i) {
    digital.spiked[0] = 1; digital.spiked[1] = 0;
    std::vector<BioReading> bio = {
      {0, 0.9f, 0, std::nanf("")},
      {1, 0.1f, 0, std::nanf("")},
    };
    tracker.Measure(digital, bio, static_cast<float>(i));
  }
  // Now diverge: digital predicts opposite of bio
  for (int i = 20; i < 40; ++i) {
    digital.spiked[0] = 0; digital.spiked[1] = 1;
    std::vector<BioReading> bio = {
      {0, 0.9f, 0, std::nanf("")},
      {1, 0.1f, 0, std::nanf("")},
    };
  ...
  assert(tracker.DriftAccelerating(20, 0.05f));
}
...
TEST(adaptive_boundary_bidirectional) {
  // Test that AutoPromoteNeighbors also promotes pre-synaptic sources
  // when a reverse adjacency list is provided
  NeuronReplacer replacer;
  replacer.Init(6);
  // Neuron 2 is monitored with low correlation (drifting)
  replacer.state[2] = NeuronReplacer::State::kMonitored;
  replacer.running_correlation[2] = 0.2f;

  // Forward: neuron 2 projects to neuron 3
  std::vector<std::vector<uint32_t>> fwd(6);
  fwd[2] = {3};
  // Reverse: neuron 0 and 1 project to neuron 2
  std::vector<std::vector<uint32_t>> rev(6);
  rev[2] = {0, 1};

  auto promoted = replacer.AutoPromoteNeighbors(fwd, 0.5f, rev);

  // Should promote neuron 3 (post-synaptic) AND neurons 0, 1 (pre-synaptic)
  assert(promoted.size() == 3);
  assert(replacer.state[0] == NeuronReplacer::State::kMonitored);
  assert(replacer.state[1] == NeuronReplacer::State::kMonitored);
  assert(replacer.state[3] == NeuronReplacer::State::kMonitored);
  // Neuron 4,5 should remain biological
  assert(replacer.state[4] == NeuronReplacer::State::kBiological);
  assert(replacer.state[5] == NeuronReplacer::State::kBiological);
}

TEST(adaptive_boundary_reverse_only) {
  // When only reverse adjacency reveals the upstream cause
  NeuronReplacer replacer;
  replacer.Init(4);
  replacer.state[2] = NeuronReplacer::State::kMonitored;
  replacer.running_correlation[2] = 0.1f;

  // No forward neighbors for neuron 2
  std::vector<std::vector<uint32_t>> fwd(4);
  // But neuron 0 feeds into neuron 2
  std::vector<std::vector<uint32_t>> rev(4);
  rev[2] = {0};
...
// ===== OptogeneticsExperiment tests =====

TEST(opto_experiment_default_brain) {
  // Run optogenetics experiment on default mouse cortical mini-column
  OptogeneticsConfig cfg;
  cfg.target_region = "L23";
  cfg.target_fraction = 0.3f;
  cfg.baseline_ms = 100.0f;
  cfg.stim_on_ms = 50.0f;
  cfg.stim_off_ms = 50.0f;
  cfg.n_pulses = 3;
  cfg.post_stim_ms = 100.0f;
  cfg.dt_ms = 0.5f;

  OptogeneticsExperiment exp;
  auto result = exp.Run(cfg);

  // Should have generated commands
  assert(result.total_commands > 0);
  // Should have measured baseline and evoked rates
  assert(result.baseline_rate_hz >= 0.0f);
  // Evoked rate should differ from baseline (light has effect)
  assert(result.evoked_rate_hz >= 0.0f);
  // Pulse spike counts should have entries
  assert(result.pulse_spike_counts.size() == 3);
  // Experiment should complete in reasonable time
  assert(result.elapsed_seconds < 30.0);
}

TEST(opto_experiment_excitatory_modulation) {
  // ChRmine excitation should increase firing rate
  OptogeneticsConfig cfg;
  cfg.excitatory_opsin = OpsinType::kChRmine;
  cfg.target_region = "L23";
  cfg.target_fraction = 0.5f;
  cfg.laser_power_mw = 15.0f;  // strong stimulation
  cfg.baseline_ms = 200.0f;
  cfg.stim_on_ms = 100.0f;
  cfg.stim_off_ms = 100.0f;
  cfg.n_pulses = 2;
  cfg.post_stim_ms = 200.0f;
  cfg.dt_ms = 0.5f;

  OptogeneticsExperiment exp;
  auto result = exp.Run(cfg);

  // Excitatory opsin should increase firing rate (positive modulation)
  // With strong stimulation, evoked should be >= baseline
  assert(result.modulation_index >= -0.5f);  // allow some tolerance
  assert(result.total_commands > 0);
}

TEST(opto_experiment_target_selection) {
  // Target neurons should be correctly selected from named region
  OptogeneticsConfig cfg;
  cfg.target_region = "L5";
  cfg.target_fraction = 0.2f;
  cfg.baseline_ms = 50.0f;
  cfg.stim_on_ms = 50.0f;
  cfg.stim_off_ms = 50.0f;
  cfg.n_pulses = 1;
  cfg.post_stim_ms = 50.0f;
  cfg.dt_ms = 0.5f;

  OptogeneticsExperiment exp;
  exp.Run(cfg);

  // Target neurons should be within L5 region range
  assert(!exp.target_neurons.empty());
  // L23 has 800 neurons (indices 0-799), L5 starts at 800
  uint32_t l5_start = 800;
  uint32_t l5_end = 1300;
  for (uint32_t idx : exp.target_neurons) {
    assert(idx >= l5_start && idx < l5_end);
  }
  // Should be ~20% of 500 = ~100 neurons
  assert(exp.target_neurons.size() >= 80 && exp.target_neurons.size() <= 120);
}

TEST(opto_experiment_safety_tracking) {
  // Cumulative energy should be tracked
  OptogeneticsConfig cfg;
  cfg.target_fraction = 0.1f;
  cfg.baseline_ms = 50.0f;
  cfg.stim_on_ms = 100.0f;
  cfg.stim_off_ms = 50.0f;
  cfg.n_pulses = 3;
  ...
  // Energy should accumulate from stimulation
  assert(result.max_energy >= 0.0f);
  // Final open fraction: after stim ends, opsin should partially recover
  // (not fully open, not fully closed)
  assert(result.final_open_fraction >= 0.0f);
  assert(result.final_open_fraction <= 1.0f);
}

TEST(opto_experiment_species_params) {
  // Mouse brain spec should use mammalian species defaults
  OptogeneticsConfig cfg;
  cfg.baseline_ms = 50.0f;
  cfg.stim_on_ms = 50.0f;
  cfg.stim_off_ms = 50.0f;
  ...
  // Default brain is mouse: check that species defaults were applied
  // Mouse tau_syn_excitatory = 3.0ms (AMPA)
  auto defaults = SpeciesDefaults::For(Species::kMouse);
  assert(std::abs(defaults.tau_syn_excitatory - 3.0f) < 0.1f);
  // Mouse STP tau_d = 200ms (Markram 1998)
  assert(std::abs(defaults.stp_tau_d - 200.0f) < 1.0f);
}

// ===== OptogeneticsOptimizer tests =====

#include "optogenetics/optimizer.h"

TEST(opto_optimizer_runs) {
  // Optimizer should complete and return trials.
  OptogeneticsOptimizer opt;
  opt.param_space = OptogeneticsOptimizer::DefaultParamSpace();

  OptimizerConfig ocfg;
  ocfg.max_trials = 5;
  ocfg.initial_random = 3;
  ocfg.population_size = 2;
  ocfg.verbose = false;

  auto result = opt.Optimize(OptoObjectives::MaxExcitation, ocfg);
  assert(result.n_trials == 5);
  assert(result.all_trials.size() == 5);
  assert(result.total_seconds > 0.0);
}

TEST(opto_optimizer_improves) {
  // Guided search should find a score >= random baseline.
  OptogeneticsOptimizer opt;
  opt.param_space = {
    {"laser_power_mw", 1.0f, 50.0f, 10.0f},
    {"target_fraction", 0.1f, 0.4f, 0.2f},
    {"stim_on_ms", 100.0f, 400.0f, 200.0f},
  };

  OptimizerConfig ocfg;
  ocfg.max_trials = 12;
  ocfg.initial_random = 4;
  ocfg.population_size = 4;
  ocfg.verbose = false;

  auto result = opt.Optimize(OptoObjectives::MaxExcitation, ocfg);
  // Best trial should have non-negative modulation (excitatory opsin)
  assert(result.best.score >= 0.0f);
  assert(result.best.result.total_commands > 0);
}

TEST(opto_optimizer_target_rate) {
  // TargetRate objective should return scores in (0, 1].
  OptogeneticsResult r;
  r.evoked_rate_hz = 50.0f;
  auto obj = OptoObjectives::TargetRate(50.0f);
  float perfect = obj(r);
  assert(std::abs(perfect - 1.0f) < 0.01f);  // exact match = 1.0

  r.evoked_rate_hz = 100.0f;
  float off = obj(r);
  assert(off < perfect);  // farther from target = lower score
  assert(off > 0.0f);
}

TEST(opto_optimizer_safe_modulation) {
  // SafeModulation penalizes high energy.
  auto obj = OptoObjectives::SafeModulation(10.0f);

  OptogeneticsResult safe;
  safe.modulation_index = 0.5f;
  safe.max_energy = 5.0f;
  assert(obj(safe) > 0.0f);

  OptogeneticsResult unsafe;
  unsafe.modulation_index = 0.8f;
  unsafe.max_energy = 15.0f;  // exceeds limit
  assert(obj(unsafe) < 0.0f);
}

TEST(opto_optimizer_param_space) {
  // OptoParam normalization round-trips correctly.
  OptoParam p{"test", 10.0f, 100.0f, 50.0f};
  float n = p.Normalize(55.0f);
  float v = p.Denormalize(n);
  assert(std::abs(v - 55.0f) < 0.01f);
  assert(std::abs(p.Clamp(5.0f) - 10.0f) < 0.01f);
  assert(std::abs(p.Clamp(200.0f) - 100.0f) < 0.01f);
}

TEST(opto_optimizer_apply_params) {
  std::vector<OptoParam> space = {
    {"laser_power_mw", 1.0f, 50.0f, 10.0f},
    {"n_pulses", 1.0f, 20.0f, 5.0f},
  };
  std::vector<float> values = {25.0f, 10.0f};
  OptogeneticsConfig cfg;
  ApplyParams(space, values, cfg);
  assert(std::abs(cfg.laser_power_mw - 25.0f) < 0.01f);
  assert(cfg.n_pulses == 10);
}
...
TEST(opto_io_result_roundtrip) {
  OptogeneticsResult orig;
  orig.baseline_rate_hz = 12.5f;
  orig.evoked_rate_hz = 45.3f;
  orig.modulation_index = 0.567f;
  orig.total_commands = 800;
  orig.max_energy = 15.7f;
  orig.elapsed_seconds = 1.23;
  orig.pulse_spike_counts = {10, 12, 8, 11, 9};

  std::string path = "test_opto_result.txt";
  assert(WriteOptoResult(path, orig));

  OptogeneticsResult loaded;
  assert(ReadOptoResult(path, loaded));

  assert(std::abs(loaded.baseline_rate_hz - orig.baseline_rate_hz) < 0.01f);
  assert(std::abs(loaded.evoked_rate_hz - orig.evoked_rate_hz) < 0.01f);
  assert(loaded.total_commands == orig.total_commands);
  assert(loaded.pulse_spike_counts.size() == 5);

  std::remove(path.c_str());
}

TEST(opto_io_config_roundtrip) {
  OptogeneticsConfig orig;
  orig.laser_power_mw = 25.5f;
  orig.target_region = "L23";
  orig.n_pulses = 8;
  orig.seed = 99;
  orig.enable_stdp = true;

  std::string path = "test_opto_config.txt";
  assert(WriteOptoConfig(path, orig));

  OptogeneticsConfig loaded;
  assert(ReadOptoConfig(path, loaded));

  assert(std::abs(loaded.laser_power_mw - 25.5f) < 0.01f);
  assert(loaded.target_region == "L23");
  assert(loaded.n_pulses == 8);
  assert(loaded.seed == 99);
  assert(loaded.enable_stdp == true);

  std::remove(path.c_str());
}

TEST(opto_io_optimizer_result_write) {
  auto space = OptogeneticsOptimizer::DefaultParamSpace();
  OptimizerResult r;
  r.n_trials = 2;
  r.total_seconds = 1.0;
  r.best.score = 0.75f;
  r.best.params = {15.0f, 0.3f, 250.0f, 400.0f, 7.0f};
  r.best.result.modulation_index = 0.6f;
  r.best.result.evoked_rate_hz = 80.0f;
  r.best.result.baseline_rate_hz = 20.0f;
  OptoTrial t1; t1.score = 0.5f;
  OptoTrial t2; t2.score = 0.75f;
  r.all_trials = {t1, t2};

  std::string path = "test_optimizer_result.txt";
  assert(WriteOptimizerResult(path, space, r));
  std::remove(path.c_str());
}
