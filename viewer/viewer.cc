...
struct FlyState {
  ...
  bool flying = false;            // true = airborne (Space held)
...
int main() {
  ...
  printf("OpenGL %d.%d loaded (brain)\n", GLAD_VERSION_MAJOR(gl_version),
  ...
  int sim_mode = 1; // 0=wave, 1=spiking, 2=fly sandbox
  std::vector<float> spike_vis(points.size(), 0.0f);
  std::vector<float> stim_extra(neurons.n, 0.0f); // persistent button stimulation
  ...
  while (!glfwWindowShouldClose(main_window)) {
    ...
    // Simulation
    if (simulate) {
      int steps = static_cast<int>(sim_speed);
      ...
      if (sim_mode == 0) {
        ...
        if (sim_mode == 2) {
          if (manual_fly) {
            ...
            constexpr float kRiseRate = 0.20f;   // per sim step (~50ms to 1.0)
            constexpr float kDecayRate = 0.10f;  // per sim step (~100ms to 0)
            ...
            smooth_flying = key_down(VK_SPACE);
            ...
            // Flying: faster speed, broader brain activation (wings need
            // coordinated oscillation across multiple neuropils).
            // Walking: slower, drives only SEZ + lateral horn.
            float speed_mult = fly.flying ? 3.0f : 1.0f;
            float base_current = fly.flying ? 18.0f : 12.0f;
            ...
            // CPG: oscillatory drive to VNC motor neurons.
            // descending_drive = forward drive from WASD (0 = stopped, 1 = walking).
            // The brain's descending commands gate the CPG on/off.
        ...
        // --- Spiking step loop ---
        // Weight scale < 1 prevents runaway excitation at rest.
        // The E/I ratio from nt_dist is correct but recurrent excitatory
        // projections can self-sustain without external damping.
        constexpr float kWeightScale = 0.4f;
        for (int s = 0; s < steps; ++s) {
          neurons.DecaySynapticInput(1.0f, 3.0f);  // tau=3ms (fast ACh)
          connectome.PropagateSpikes(neurons.spiked.data(),
                                     neurons.i_syn.data(), kWeightScale);
          IzhikevichStep(neurons, 1.0f, spike_time, izh_params);
          ...
          spike_time += 1.0f;
          // Decay button stimulation (~200ms half-life at 1ms steps)
          for (size_t i = 0; i < neurons.n; ++i)
            stim_extra[i] *= 0.997f;

          // Motor output: update every brain step (1ms).
          motor.Update(neurons, 1.0f);
          if (g_body.ready) {
            auto cmd = motor.Command();
            static float smooth_fwd_vel = 0.0f;
            static float smooth_ang_vel = 0.0f;
            constexpr float kBodyTau = 15.0f;  // snappy response
            float body_alpha = 1.0f - std::exp(-1.0f / kBodyTau);
            smooth_fwd_vel += body_alpha * (cmd.forward_velocity - smooth_fwd_vel);
            smooth_ang_vel += body_alpha * (cmd.angular_velocity - smooth_ang_vel);
            if (cmd.freeze > 0.5f) { smooth_fwd_vel = 0.0f; smooth_ang_vel = 0.0f; }
            double magnitude = std::clamp(
                static_cast<double>(smooth_fwd_vel) / 10.0, 0.0, 1.5);
            double turn = std::clamp(
                static_cast<double>(smooth_ang_vel) / 0.8, -1.0, 1.0);
#ifdef FWMC_BODY_SIM
            g_body.cpg.SetDrive(magnitude, turn);
#endif
          }
        }
        spike_count_display = neurons.CountSpikes();

        // Body physics: one step per frame (decoupled from brain rate).
#ifdef FWMC_BODY_SIM
        if (g_body.ready) g_body.Step();
#endif

        // --- Fly sandbox: read motor output and move fly ---
        if (sim_mode == 2) {
          // Instantaneous firing rate per region (spikes this step / n_neurons)
          auto inst_rate = [&](int reg) -> float {
            ...
            // Convert to Hz: multiply by (1000 ms / dt_ms)
            return total > 0 ? static_cast<float>(count) / total * 1000.0f : 0.0f;
          };

          float alpha = 0.07f;  // EMA smoothing
          ...
          // Kinematics: forward from SEZ, turn from lateral horn asymmetry
          // Flying triples speed and loosens turning
          float fly_mult = fly.flying ? 3.0f : 1.0f;
          float fwd  = std::clamp(fly.rate_sez * kFlySpeedScale * fly_mult, -2.0f, 24.0f);
          float turn = (fly.rate_lh_L - fly.rate_lh_R) * kFlyTurnScale * (fly.flying ? 1.5f : 1.0f);
          if (fly.flying) fly.wing_phase += 0.5f;  // animate wings
        ...
        // TCP bridge to external nmfly body (optional, if standalone body sim)
        if (tcp_enabled) {
          auto cmd = motor.Command();
        ...
        for (int r = 0; r < 13; ++r) {
          ...
          region_rates[r] = frame_counts[r] * 1000.0f; // Hz (1ms steps)
        }
    ...
    if ((sim_mode == 1 || sim_mode == 2) &&
      ...
      // STDP toggle
      ImGui::Separator();
      ImGui::Checkbox("STDP Learning", &stdp_enabled);
      if (stdp_enabled) {
    ...
    // ----------------------------------------------------------------
    // Fly Sandbox arena panel (left half, mode 2 only)
    // ----------------------------------------------------------------
    if (false && sim_mode == 2) {  // disabled: top-down arena not needed
      int fb_w_now, fb_h_now;
      glfwGetFramebufferSize(window, &fb_w_now, &fb_h_now);

      ImGui::SetNextWindowPos(ImVec2(0, 0), ImGuiCond_Always);
      ImGui::SetNextWindowSize(ImVec2(460, static_cast<float>(fb_h_now)), ImGuiCond_Always);
      ...
      const float cw = 420.0f, ch = 420.0f;
      ImVec2 cp = ImGui::GetCursorScreenPos();
      ImDrawList* dl = ImGui::GetWindowDrawList();

      // Arena floor
      dl->AddRectFilled(cp, ImVec2(cp.x + cw, cp.y + ch), IM_COL32(12, 18, 12, 255), 6.0f);
      dl->AddRect(cp, ImVec2(cp.x + cw, cp.y + ch), IM_COL32(50, 70, 50, 255), 6.0f, 0, 2.0f);

      // Helper: arena [0..kArenaSize] → screen pixels
      auto a2s = [&](float ax, float ay) -> ImVec2 {
        return ImVec2(cp.x + ax * cw / kArenaSize,
                      cp.y + ay * ch / kArenaSize);
      };

      // Odor plumes — three concentric rings with alpha falloff
      for (auto& src : odor_sources) {
        ImVec2 sc = a2s(src.x, src.y);
        float  pr = 90.0f * src.strength;
        auto col  = [&](float alpha) -> ImU32 {
          return IM_COL32((int)(src.r*255), (int)(src.g*255),
                          (int)(src.b*255), (int)(alpha*255));
        };
        dl->AddCircleFilled(sc, pr * 0.28f, col(0.35f));
        dl->AddCircle(sc, pr * 0.55f, col(0.18f), 40, 1.5f);
        dl->AddCircle(sc, pr,         col(0.08f), 40, 1.0f);
        // Source dot
        dl->AddCircleFilled(sc, 5.0f, col(1.0f));
      }

      // Fly trail
      if (fly.trail_count > 1) {
        for (int i = 1; i < fly.trail_count; ++i) {
          int i0 = (fly.trail_head - fly.trail_count + i - 1 + FlyState::kTrailLen) % FlyState::kTrailLen;
          int i1 = (fly.trail_head - fly.trail_count + i     + FlyState::kTrailLen) % FlyState::kTrailLen;
          float t = static_cast<float>(i) / fly.trail_count;
          dl->AddLine(a2s(fly.trail_x[i0], fly.trail_y[i0]),
                      a2s(fly.trail_x[i1], fly.trail_y[i1]),
                      IM_COL32(180, 180, 180, (int)(t * 70)), 1.5f);
        }
      }

      // Fly body
      float sc_fx = cp.x + fly.x * cw / kArenaSize;
      float sc_fy = cp.y + fly.y * ch / kArenaSize;
      float h  = fly.heading;

      if (fly.flying) {
        // Airborne: larger body with animated wings
        float sz = 14.0f;
        float wing_flap = std::sin(fly.wing_phase) * 0.4f; // wing beat

        // Body (elongated)
        ImVec2 nose(sc_fx + std::cos(h) * sz, sc_fy + std::sin(h) * sz);
        ImVec2 tail(sc_fx - std::cos(h) * sz * 0.7f, sc_fy - std::sin(h) * sz * 0.7f);
        dl->AddLine(nose, tail, IM_COL32(240, 220, 175, 255), 3.0f);

        // Wings (flapping)
        float wing_angle_L = h + 1.57f + wing_flap;
        float wing_angle_R = h - 1.57f - wing_flap;
        float wing_len = sz * 1.2f;
        ImVec2 wL(sc_fx + std::cos(wing_angle_L) * wing_len,
                  sc_fy + std::sin(wing_angle_L) * wing_len);
        ImVec2 wR(sc_fx + std::cos(wing_angle_R) * wing_len,
                  sc_fy + std::sin(wing_angle_R) * wing_len);
        ImVec2 center(sc_fx, sc_fy);
        dl->AddLine(center, wL, IM_COL32(200, 210, 255, 180), 2.0f);
        dl->AddLine(center, wR, IM_COL32(200, 210, 255, 180), 2.0f);
        // Wing tips
        dl->AddCircleFilled(wL, 3.0f, IM_COL32(200, 210, 255, 120));
        dl->AddCircleFilled(wR, 3.0f, IM_COL32(200, 210, 255, 120));

        // Shadow on ground
        dl->AddCircleFilled(ImVec2(sc_fx + 4, sc_fy + 4), 8.0f,
                            IM_COL32(0, 0, 0, 40));
      } else {
        // Walking: triangle body with legs
        float sz = 11.0f;
        ImVec2 tip  (sc_fx + std::cos(h)        * sz,        sc_fy + std::sin(h)        * sz);
        ImVec2 legL(sc_fx + std::cos(h + 2.3f) * sz * 0.6f, sc_fy + std::sin(h + 2.3f) * sz * 0.6f);
        ImVec2 legR(sc_fx + std::cos(h - 2.3f) * sz * 0.6f, sc_fy + std::sin(h - 2.3f) * sz * 0.6f);
        dl->AddTriangleFilled(tip, legL, legR, IM_COL32(240, 220, 175, 255));
        dl->AddTriangle(tip, legL, legR, IM_COL32(255, 255, 255, 160), 1.5f);

        // 3 leg pairs (alternating tripod gait hint)
        float leg_len = sz * 0.5f;
        for (int li = 0; li < 3; ++li) {
          float along = -0.2f + 0.3f * li; // position along body axis
          float bx = sc_fx + std::cos(h) * sz * along;
          float by = sc_fy + std::sin(h) * sz * along;
          ImVec2 lL(bx + std::cos(h + 1.5f) * leg_len,
                    by + std::sin(h + 1.5f) * leg_len);
          ImVec2 lR(bx + std::cos(h - 1.5f) * leg_len,
                    by + std::sin(h - 1.5f) * leg_len);
          ImVec2 bc(bx, by);
          dl->AddLine(bc, lL, IM_COL32(180, 160, 120, 160), 1.0f);
          dl->AddLine(bc, lR, IM_COL32(180, 160, 120, 160), 1.0f);
        }
      }

      // Antennae — color encodes odor concentration
      float ant_len = cw / kArenaSize * 18.0f;
      ImVec2 fly_c(sc_fx, sc_fy);
      ImVec2 ant_L(sc_fx + std::cos(h + 0.5f) * ant_len,
                   sc_fy + std::sin(h + 0.5f) * ant_len);
      ImVec2 ant_R(sc_fx + std::cos(h - 0.5f) * ant_len,
                   sc_fy + std::sin(h - 0.5f) * ant_len);
      dl->AddLine(fly_c, ant_L,
                  IM_COL32(80 + (int)(fly.odor_L * 175), 255, 80, 220), 2.0f);
      dl->AddLine(fly_c, ant_R,
                  IM_COL32(80 + (int)(fly.odor_R * 175), 255, 80, 220), 2.0f);

      // Reserve canvas space
      ImGui::Dummy(ImVec2(cw, ch));

      // Sensor / motor readouts
      ImGui::Separator();
      ImGui::Text("Olfactory   L: %.2f   R: %.2f", fly.odor_L, fly.odor_R);
      ImGui::Text("Lateral Horn  L: %5.0f Hz   R: %5.0f Hz",
                  fly.rate_lh_L, fly.rate_lh_R);
      ImGui::Text("SEZ (fwd drive):  %5.0f Hz", fly.rate_sez);
      ImGui::Spacing();

      // Odor source controls
      ImGui::SeparatorText("Odor Sources");
      for (int i = 0; i < (int)odor_sources.size(); ++i) {
        ImGui::PushID(i);
        float cr = odor_sources[i].r, cg = odor_sources[i].g, cb = odor_sources[i].b;
        ImGui::ColorButton("##col", ImVec4(cr, cg, cb, 1.0f),
                           ImGuiColorEditFlags_NoTooltip, ImVec2(14, 14));
        ImGui::SameLine();
        ImGui::SetNextItemWidth(90);
        ImGui::SliderFloat("##x", &odor_sources[i].x, 0, kArenaSize, "X%.0f");
        ImGui::SameLine();
        ImGui::SetNextItemWidth(90);
        ImGui::SliderFloat("##y", &odor_sources[i].y, 0, kArenaSize, "Y%.0f");
        ImGui::SameLine();
        ImGui::SetNextItemWidth(70);
        ImGui::SliderFloat("##s", &odor_sources[i].strength, 0.0f, 2.0f, "%.1f");
        ImGui::PopID();
      }

      // Manual control toggle
      ImGui::SeparatorText("Control");
      ImGui::Checkbox("WASD Control", &manual_fly);
      if (manual_fly) {
        ImGui::TextColored(ImVec4(0.6f, 1.0f, 0.6f, 1.0f), "W/S");
        ImGui::SameLine(); ImGui::TextUnformatted("Walk forward / back");
        ImGui::TextColored(ImVec4(0.6f, 1.0f, 0.6f, 1.0f), "A/D");
        ImGui::SameLine(); ImGui::TextUnformatted("Turn left / right");
        ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.4f, 1.0f), "Space");
        ImGui::SameLine(); ImGui::TextUnformatted("Hold to fly");
        if (fly.flying) {
          ImGui::TextColored(ImVec4(0.5f, 0.7f, 1.0f, 1.0f), "  AIRBORNE");
        }
      }

      ImGui::Spacing();
      if (ImGui::Button("Reset Fly")) {
        fly.Reset(kArenaSize * 0.5f, kArenaSize * 0.5f);
        std::fill(stim_extra.begin(), stim_extra.end(), 0.0f);
        neurons.ClearExternalInput();
        for (size_t i = 0; i < neurons.n; ++i) {
