...
# Run tests (179 in test_core; test_bridge/test_parametric/test_tissue also available)
...
| **Experiments** | | |
| `--visual-escape` | off | 02: Visual looming escape response |
| `--navigation` | off | 03: CX ring attractor heading tracking |
| `--whisker` | off | 04: Mouse barrel cortex whisker response |
| `--prey-capture` | off | 05: Zebrafish prey capture |
| `--courtship` | off | 06: Drosophila courtship song generation |
| `--twinning` | off | 07: Full neural twinning demo (train + replace + test) |
| `--ablation` | off | 08: Progressive KC ablation degradation curve |
| `--compensated` | off | 09: Compensated ablation (digital twin prosthesis demo) |
| `--test-all` | off | Run all 9 experiments as regression suite |
| `--nav-optimize` | off | Auto-tune navigation circuit via ExperimentOptimizer |
| `--escape-optimize` | off | Auto-tune visual escape circuit |
| **Parametric extras** | | |
...
```
../mechabrain/                 All mechanistic brain code (header-only C++23)
  ...
  experiments/                9 structured neuroscience experiments
  optogenetics/               Opsin kinetics, stimulation optimization, hardware I/O
src/
  fwmc.cc                     Entry point and CLI
  experiment_runner.h         Top-level experiment orchestrator
  bridge_self_test.h          Software validation of bridge pipeline
  multi_trial.h               Multi-seed aggregate analysis
...
See [mechabrain/README.md](../mechabrain/README.md) for detailed module documentation.
...
Pipeline: build, run tests (4 suites: test_core, test_bridge, test_parametric, test_tissue), run benchmarks, generate test data, smoke test full simulation. Results cached by CMake build directory.
