...
## 3.5 Current In-Silico Results

The following results have been demonstrated in simulation using FWMC's experiment suite. These validate the computational feasibility of each phase before biological experiments begin.

### Phase 0 Results: Parametric Calibration

- **Olfactory conditioning** (01): Mushroom body circuit (690 neurons, 5K synapses) trained via dopamine-gated STDP with eligibility traces. Learning index = 4.93, KC->MBON weight ratio = 10x. Circuit learns associative CS+/US pairing in 5 trials.
- **Multi-species validation**: Same engine runs mouse barrel cortex (2380 neurons, 122K synapses, L4 onset 28.1ms) and zebrafish prey capture (5000 neurons, 356K synapses, tectal onset 98.6ms), confirming the parametric generator scales beyond Drosophila.
- **Auto-tuned ring attractor**: CX navigation circuit (116 neurons) with heading error 2.5 degrees, dark persistence 8.7 degrees, R2=0.998 vs visual stimulus. Parameters found automatically by ExperimentOptimizer with multi-seed robustness.

### Phase 5 Results: Graceful Degradation (In Silico)

Two experiments directly address Phase 5:

**Ablation study (08)**: Progressive silencing of 0-90% of Kenyon cells after STDP training. The mushroom body exhibits graceful degradation: continuity (MBON response / baseline) remains above 50% even at 90% KC ablation (graceful score = 0.86). This confirms sparse KC coding robustness (Hige et al. 2015) and calibrates the replacement rate budget: the circuit can tolerate substantial neuron loss before behavioral failure.

**Compensated ablation (09)**: The headline prosthesis result. Compares two conditions at each ablation level:
- **Pure ablation**: silence KC neurons, measure MBON response degradation
- **Compensated**: silence the same neurons, but a digital twin (using trained weights) fills in for lost neurons, injecting its spikes into the surviving circuit

| Ablation | Pure continuity | Compensated continuity | Benefit |
|----------|----------------|----------------------|---------|
| 10% | 1.03 | 1.15 | +0.12 |
| 50% | 0.89 | 1.09 | +0.20 |
| 70% | 0.76 | 0.95 | +0.19 |
...
Key findings:
- The compensated circuit **never drops below 50% continuity**, even at 90% ablation (pure ablation crosses 50% at 90%)
- Compensation benefit **increases with damage severity**: +0.12 at 10% ablation, +0.29 at 90%
- The digital twin extends functional circuit lifetime effectively infinitely -- the compensated half-life is never reached
- Graceful scores: pure = 0.85, compensated = 1.02

This demonstrates the core neural prosthesis principle: a digital twin with trained weights can sustain learned behavior as biological neurons fail. The sicker the brain, the more the prosthesis helps.

**Twinning demo (07)**: Full shadow-tracking and closed-loop replacement pipeline. After training (LI=4.9), the digital twin preserves 100% of CS+ MBON response (2343 spikes). When 10% of bio neurons are killed, the twin maintains full behavioral continuity (1.000). Spike-level correlation is ~0 in chaotic spiking networks; behavioral magnitude is the meaningful continuity metric.
...
- **179 unit tests** across core, bridge, parametric, and tissue systems; benchmarks for performance regression
- **9 experiment regression tests** covering conditioning, visual escape, navigation, whisker, prey capture, courtship, twinning, ablation, and compensated ablation (all passing)
